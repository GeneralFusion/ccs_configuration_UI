    ...
    return (
      <Paper elevation={4}>
        <Box sx={styles.smallBox}>
          <h2 style={styles.smallBoxH2}>
            {props.propertiesDB[props.property].name}
          </h2>
          {valueChanger}
        </Box>
      </Paper>
...
function createValueChanger(props) {
  ...
  const levelTooLow =
    props.userLevel < 2 ||
    props.userLevel < props.propertiesDB[props.property]["permission"]
    //If user does not have permission 2 then there is no need to check anything else. They have no write permsision for anything.
  switch (
